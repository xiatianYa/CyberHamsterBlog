<template>
    <main class="main">
        <div class="list" :class="weather ? 'backgroundcolor-fffffb' : 'backgroundcolor-130c0e'">
            <div class="classify">
                <div class="wrapper">
                    <h2 :class="weather ? 'color-130c0e' : 'color-fffffb'">归档
                        <span>{{ this.$route.query.classifyName }}</span>
                        下的所有文章
                    </h2>
                    <div class="list_item">
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            blogs: (state) => state.classify.blogs,
        }),
        noMore() {
    ...
    mounted() {
        this.$store.dispatch("getBlogsByClassId", this.$route.query.classifyId);
    },
    watch: {
        $route(to, from) {
            this.$store.dispatch("getBlogsByClassId", this.$route.query.classifyId);
